...
if ( !class_exists( 'ReduxFramework_extension_demo_importer' ) ) {
	...
	class ReduxFramework_extension_demo_importer {
		...
		public function __construct( $parent ) {
			...
			include URAY_THEME_DIR . '/inc/admin/demo_importer/inc/class-demo-importer-progress.php';
					...
					if ( $import['type'] == "d" && !empty( $import['name'] ) ) {
						...
						foreach ( $import['files'] as $file ) {
							switch ( $file['name'] ) {
								...
								case 'content.xml':
								case 'content-el.xml':
									$file_content = 'content.xml';
									if (  is_plugin_active( 'elementor/elementor.php' ) ){
										$file_content = 'content-el.xml';
									}
									$this->data_import_files['data-import-' . $x]['content_file'] = $file_content;
		...
		public function ajax_importer() {
			...
			if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->data_import_files ) ) {
				...
				if ( !isset( $import_parts['imported'] ) || true === $reimporting ) {
					include URAY_THEME_DIR . '/inc/admin/demo_importer/inc/init-installer.php';
					$installer = new Radium_Theme_Demo_Data_Importer( $this, $this->parent );
				} else {
					echo esc_html__( "Demo Already Imported", 'uray' );
				}
		...
		function add_importer_section() {
			...
			$demo_importer_label = trim( apply_filters( 'demo_importer_label', esc_html__( 'Demo Importer', 'uray' ) ) );

			$demo_importer_label = ( !empty( $demo_importer_label ) ) ? $demo_importer_label : esc_html__( 'Demo Importer', 'uray' );

			$this->parent->sections[] = array(
				'id'     => 'demo_importer_section',
				'title'  => $demo_importer_label,
				'desc'   => '<p class="description">' . apply_filters( 'demo_importer_description', esc_html__( 'Works best to import on a new install of WordPress', 'uray' ) ) . '</p>',
